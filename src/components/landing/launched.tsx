            ...
            {rows.map((row) => {
              ...
              return (
                <li key={row.pool}>
                  <Link
                    href={href}
                    className="flex h-full flex-col justify-between rounded-3xl bg-[#FFF6E8] p-5 text-[#2A1A14]"
                  >
                    <div>
                      <p className="font-spine text-[11px] tracking-[0.16em] text-[#1F7A78] uppercase">
                        {stock.ticker} · live
                      </p>
                      <h3 className="font-letter mt-2 text-2xl leading-tight">{row.symbol}</h3>
                      <p className="mt-1 text-sm text-[#2A1A14]/70">{sku.name}</p>
                    </div>
